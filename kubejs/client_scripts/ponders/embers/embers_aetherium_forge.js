	...
	event.create([
		...
		.scene("embers_aetherium_forge", "Operating the Aetherium Forge", "kubejs:embers_aetherium_forge",
			(scene, util) => {
				...
				let pEmber = new $SparkParticleOptions($EmbersColors.EMBER_ID, 1.0);
				let pSmoke = new $SmokeParticleOptions($EmbersColors.SMOKE_ID, 3.0);
				let emitEmber = scene.effects.simpleParticleEmitter(pEmber, util.vector.of(0.1, 0, 0.1));
				let emitSmoke = scene.effects.simpleParticleEmitter(pSmoke, util.vector.of(0.1, 0, 0.1));
