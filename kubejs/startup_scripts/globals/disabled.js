global.disabledItems = [
	'draconicevolution:generator',
	'ad_astra:coal_generator',
	'industrialforegoing:pitiful_generator',
	
	'ftbquests:loot_crate_opener',
	'/ftbquests:screen_.+/',
	'ftbquests:task_screen_configurator',
	//C-DECO
	'createdeco:netherite_nugget',
	//TOOLS-CCOMPLEMENT
	'/tools_complement:(silver|lead)_(sword|pickaxe$|axe$|shovel|hoe)/',
	'/tools_complement:silver_(helmet|chestplate|leggings|boots)/',
	'/tools_complement:copper_chestplate/',
	'/tools_complement:copper_sword/',
	//GOBBER
	'gobber2:gobber2_ring_attraction', 
	'gobber2:gobber2_ring_repair', 
	'/gobber2:gobber2_ring_vision/', 
	'gobber2:gobber2_ring_curing', 
	'gobber2:gobber2_ring_enderchest', 
	'gobber2:gobber2_staff_ensnarement', 
	'gobber2:gobber2_medallion_hero', 
	'gobber2:gobber2_medallion_breathing', 
	//INTDYN
	'integrateddynamics:coal_generator',
	//EIO
	'enderio:energetic_photovoltaic_module', 
	'enderio:pulsating_photovoltaic_module', 
	'enderio:vibrant_photovoltaic_module',
	'enderio:stirling_generator',
	//ARTIFACTS
	'artifacts:everlasting_beef',
	'artifacts:eternal_steak',
	//MINI-U
	'miniutilities:golden_lasso',
	'miniutilities:magical_egg',
	//NA
	'naturesaura:chunk_loader',
	//EIDOLON
	'/eidolon:silver_sword/',
	'/eidolon:silver_pickaxe/',
	'/eidolon:silver_axe/',
	'/eidolon:silver_shovel/',
	'/eidolon:silver_hoe/',
	//GAVELS
	'/forbidden_arcanus:wooden_blacksmith_gavel/',
	'/forbidden_arcanus:golden_blacksmith_gavel/',
	//DUPLICATE-TROUGH
	'farmingforblockheads:feeding_trough',
	//SILICON
	'tinyredstone:silicon_compound'
];

let disabledEnchantments = [
	'enderzoology:soulbound',
	'enderio:soulbound',
	'ars_elemental:soulbound',
	'miniutilities:molten_head',
	'miniutilities:shotgun',
	'miniutilities:experience_harvester'
];

disabledEnchantments.forEach(e => {
	global.disabledItems.push(Item.of('minecraft:enchanted_book','{StoredEnchantments:[{id:"'+e+'"}]}').weakNBT())
})

global.clearedItems = [
	'botania:spark',
	
	'#forge:ingots/gobber',
	'#forge:ingots/gobber_nether',
	'#forge:ingots/gobber_end',

	'miniutilities:ender_lily_seeds',
	'forbidden_arcanus:corrupti_dust',
	'enigmaticlegacy:infinimeal',
	'scannable:scanner',
	'tiab:time_in_a_bottle',
	
	'/functionalstorage:(?!framed|fluid).+(2|4)/',

	'ars_nouveau:novice_spell_book',

	'handcrafted:wood_plate', 
	'cookingforblockheads:spice_rack', 
	'luphieclutteredmod:small_shelf',

	'thermal:white_rockwool',

	'functionalstorage:framed_1', 
	'functionalstorage:framed_2', 
	'functionalstorage:framed_4', 
	'functionalstorage:compacting_framed_drawer', 
	'functionalstorage:framed_storage_controller', 
	'functionalstorage:framed_controller_extension', 
	'functionalstorage:framed_simple_compacting_drawer',

	'immersive_aircraft:improved_landing_gear',
	'immersive_aircraft:eco_engine',
	'immersive_aircraft:industrial_gears',
	'immersive_aircraft:sturdy_pipes',
	
	'eidolon:basic_amulet', 
	'eidolon:basic_ring', 
	'eidolon:basic_belt',
	
	'enigmaticlegacy:mending_mixture',
	'enigmaticlegacy:magnet_ring',
	'enigmaticlegacy:golden_ring',
	'enigmaticlegacy:enchantment_transposer',
	'enigmaticlegacy:extradimensional_eye',
	
	'toms_storage:ts.trim',
	'toms_storage:ts.inventory_proxy',
	'toms_storage:ts.open_crate',
	'toms_storage:ts.inventory_hopper_basic',
	'toms_storage:ts.level_emitter',
	'toms_storage:ts.inventory_connector',
	'toms_storage:ts.storage_terminal',
	'toms_storage:ts.crafting_terminal',
	'toms_storage:ts.inventory_cable_connector',
	'toms_storage:ts.wireless_terminal',
	'toms_storage:ts.inventory_cable',
	'toms_storage:ts.inventory_cable_framed',
	
	'miniutilities:iron_opinium_core', 
	'miniutilities:gold_opinium_core', 
	'miniutilities:diamond_opinium_core', 
	'miniutilities:netherite_opinium_core', 
	'miniutilities:emerald_opinium_core', 
	'miniutilities:chorus_opinium_core', 
	'miniutilities:experience_opinium_core', 
	'miniutilities:nether_star_opinium_core', 
	'miniutilities:the_final_opinium_core'
];

//DISABLED
global.disabledTooltip = [
	'ae2:spatial_anchor',
	'pneumaticcraft:chunkloader_upgrade',
	'miniutilities:quantum_quarry',
	'miniutilities:chorus_tile',
	'miniutilities:ender_tile',
	'miniutilities:wooden_spikes', 
	'miniutilities:iron_spikes', 
	'miniutilities:gold_spikes', 
	'miniutilities:diamond_spikes', 
	'miniutilities:netherite_spikes',
	'miniutilities:solar_panel_controller', 
	'miniutilities:solar_panel', 
	'miniutilities:lunar_panel'
];