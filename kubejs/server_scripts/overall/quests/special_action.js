...
global.MobEffectAdded = (event, effectId) => {
	...
	if (effectId == "cold_sweat:ice_resistance") simpleQuestComplete(player, "033E156124085BAD");
}
...
EntityEvents.hurt(event => {
	...
	//TREMORSAURUS STUN
	if (source?.mainHandItem == Item.of('alexscaves:primitive_club') && entity.getType() == 'alexscaves:tremorsaurus') {
	...
	//GINGERBREAD MAN
	if (entity.getType() == 'alexscaves:gingerbread_man') {
...
EntityEvents.death(event => {
	...
	//FLESH BLOB KILL
	if (['biomancy:flesh_blob', 'biomancy:hungry_flesh_blob', 'biomancy:legacy_flesh_blob', 'biomancy:primordial_flesh_blob', 'biomancy:primordial_hungry_flesh_blob'].includes(entity.getType())) {
...
global.AnvilApply = event => {
	...
	if (!hasCompletedQuest(entity, '0DC2F94E32B5B42F') && /.+elementalcraft:\{jewel:.+\}.+/.test(output.nbt)) {
